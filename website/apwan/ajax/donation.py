...
def create(request, recipient_id, entity_id, amount):
    ...
    if not success:
        return cors_response(build_error(ERROR.INVALID_PARAMETER, parameter='recipient_id'))

    entity_id, success = validate_int(entity_id)
    if not success:
        return cors_response(build_error(ERROR.INVALID_PARAMETER, parameter='entity_id'))

    amount, success = validate_float(amount)
    if not success:
        return cors_response(build_error(ERROR.INVALID_PARAMETER, parameter='amount'))
    if amount <= 0:
        return cors_response(build_error(ERROR.INVALID_PARAMETER, parameter='amount'))
    ...
    if payee is None or payee.user is None:
        return cors_response(build_error(ERROR.DONATION.NO_PAYEE))
